...
| 2026-02-22 | `sudo unshare --net ./my_tool` | Runs `my_tool` in an isolated network namespace with no interfaces, routes, or connectivity - useful for testing whether a tool works without network access. The namespace is kernel-level and disappears automatically when the process exits. | linux, networking, testing |
| 2026-02-23 | `pgrep -la pattern` | List all running processes matching a name pattern, with their PIDs and full command lines. `pkill pattern` kills them all. Much friendlier than `ps aux \| grep ...`. | linux, shell, productivity |
...
| 2026-02-28 | Use `dplyr::add_count(df, dplyr::across(everything())) \|> dplyr::filter(n > 1)` to find duplicate rows. | `dplyr::add_count()` appends an `n` column counting identical rows without collapsing the data; `dplyr::across(everything())` groups by all columns so only true duplicates get `n > 1`. | rstats, dplyr |
| 2026-03-01 | Use `dplyr::case_when()` for vectorised if-else logic. | `dplyr::case_when(x > 10 ~ "high", x > 5 ~ "medium", .default = "low")` evaluates conditions in order and returns the corresponding value; the `.default` argument handles all unmatched cases. | rstats, dplyr |
| 2026-03-02 | Use `dplyr::anti_join()` to filter out a blocklist. | `dplyr::anti_join(users, banned, by = "user_id")` removes every banned user in one readable step, without needing `%in%` or a manual `filter(!id %in% other$id)`. | rstats, dplyr |
| 2026-03-03 | Prefer `dplyr::if_else(x > 0, "positive", "non-positive")` over base `ifelse()`. | `dplyr::if_else()` enforces that the `true` and `false` values are the same type, catching subtle bugs; it also handles `NA` explicitly via the `missing` argument. | rstats, dplyr |
| 2026-03-04 | `zgrep "ERROR" app.log.gz` | Searches inside a gzipped file without decompressing it first. The whole `z*` family works the same way: `zcat`, `zless`, `zdiff`, and `zawk` all accept `.gz` files directly. | shell, linux, productivity |
| 2026-03-05 | `find dir1 dir2 dir3 -name "*.txt"` | `find` accepts multiple starting paths; fewer people know this than other `find` tricks, and it avoids running the command multiple times or using a loop. | shell, find |
| 2026-03-06 | Use `systemctl --user enable --now myservice.service` to run your own services without root. | Place a unit file in `~/.config/systemd/user/` and `systemctl --user` manages it entirely in your own session — no sudo needed. Use `systemctl --user status` to inspect all your personal services. | shell, linux, systemd |
| 2026-03-07 | Use `disown %1` to detach a background job so it survives logout. | After running `cmd &`, use `jobs` to see job numbers, then `disown %1` removes job 1 from the shell's job table. Unlike `nohup`, it works on already-running jobs you forgot to protect. | shell, linux, productivity |
| 2026-03-08 | Use `dplyr::na_if(x, "")` to convert a sentinel value to `NA`. | `dplyr::na_if(x, -999)` or `dplyr::na_if(x, "")` replaces a known placeholder with a proper `NA` in one step, avoiding a verbose `mutate(x = ifelse(x == "", NA, x))`. | rstats, dplyr |
| 2026-03-09 | `swapon --show` displays all active swap spaces and their usage. | The output columns are: `NAME` (device or file path), `TYPE` (partition or file), `SIZE` (total swap space), `USED` (currently occupied), and `PRIO` (priority — higher values are used first; ties go to the earlier-activated swap). | shell, linux, sysadmin |
| 2026-03-10 | Use `Ctrl-a M` in GNU screen to monitor a background window for activity. | Screen highlights the window and alerts you when new output appears, so you can work elsewhere without constantly switching to check on a running job. Press `Ctrl-a M` again in the same window to toggle monitoring off. | screen, productivity |
| 2026-03-11 | `all(x %% 1 == 0)` | In R, `is.integer()` will return `FALSE` even if you have all integers but stored in class numeric; use `all(x %% 1 == 0)`. | rstats |
| 2026-03-12 | Use `sudo journalctl -k --grep="oom_reaper"` to find kernel OOM kill events. | `journalctl -k` reads kernel messages from the persistent systemd journal and `--grep` filters entries by a regex pattern applied to the message field. | shell, linux, sysadmin |
| 2026-03-13 | Use `.groups = "drop"` in `dplyr::summarise()` to avoid silently retaining grouping. | After `group_by() \|> summarise()`, the result remains grouped by all but the last grouping variable; passing `.groups = "drop"` makes the output a plain data frame and prevents subtle bugs in downstream operations. | rstats, dplyr |
| 2026-03-14 | Prefer `dplyr::bind_rows()` over `rbind()` for stacking data frames. | `bind_rows()` matches columns by name rather than position, and fills any missing columns with `NA` instead of erroring; it also accepts a list of data frames and an `.id` argument to record which source each row came from. | rstats, dplyr |
| 2026-03-15 | Use `dplyr::consecutive_id()` to label consecutive runs of identical values. | `dplyr::mutate(df, run = dplyr::consecutive_id(x))` increments the ID each time `x` differs from the previous row. | rstats, dplyr |
| 2026-03-16 | Use `readr::cols_only()` to read a subset of columns from a CSV. | `readr::read_csv(f, col_types = cols_only(id = col_character(), value = col_double()))` reads only the columns named in `cols_only()`; they must exist in the CSV header or you get an error. Everything else is silently dropped. | rstats, readr |
| 2026-03-17 | Use `readr::read_csv(f, name_repair = "universal")` to auto-fix messy column names. | The `"universal"` strategy applies `vctrs::vec_as_names()` with `repair = "universal"`, which replaces spaces and special characters with `.`, prepends `...` to names that start with a number, and appends `...N` to disambiguate duplicates — guaranteeing every column name is unique, syntactically valid, and usable without backticks. | rstats, readr |
| 2026-03-18 | Use `ssh-keygen -R hostname` to remove a stale host key from `known_hosts`. | When a SSH key changes, SSH refuses to connect with a scary "host key verification failed" warning. `ssh-keygen -R hostname` deletes only that host's entry from `~/.ssh/known_hosts`. | shell, linux, ssh |
| 2026-03-19 | Use `git add -u` to stage only modified and deleted files, skipping untracked files. | Unlike `git add -A` or `git add .`, which stage everything including new untracked files, `git add -u` restricts staging to files already tracked by Git. This is useful when you have scratch files, logs, or experiments in your working tree that you do not want to accidentally commit. | git |
| 2026-03-20 | Use `git checkout origin/main -- path/to/file` to grab a single updated file from the remote. | `git pull` always fetches and merges entire commits. To update just one file, run `git fetch origin` first, then `git checkout origin/main -- path/to/file` to overwrite your local copy with the remote version without merging everything else. | git |
| 2026-03-21 | Use a Git worktree when you need to context-switch without disrupting your current in-progress work. | By default, a Git repo has one worktree (your main checkout). With `git worktree add ../hotfix hotfix-branch`, you create an additional directory on disk that checks out a different branch. Each worktree has its own files and index but shares the same Git history, so there is no need to stash or commit before switching tasks. | git, productivity |
| 2026-03-22 | Think on the margin: base decisions on the additional cost and benefit of one more unit, not on totals or averages. | Marginal thinking asks "should I do one more?" rather than "should I do this at all?" For example, whether to work an extra hour depends on what that specific hour yields versus what it costs you, not on how many hours you have already put in. Sunk costs are irrelevant; only the next unit matters. | economics |
| 2026-03-23 | Use `cut -f2-` to extract from the second field onwards. | The `-f` flag in `cut` accepts range specifiers: `2-` means field 2 through the last field, `1-3` means fields 1 to 3, and `-4` means fields 1 to 4. | shell, reminder |
| 2026-03-24 | Use `my_list[[1]]` as the first argument and `my_list[-1]` as the second to pass a list to a function that expects a single element plus a list. | When a function like `merge()` takes one object as `x` and a list of objects as `y`, use `merge(my_list[[1]], my_list[-1])` to split a list into its first element and the remainder using negative indexing, avoiding loops or repeated calls. | rstats |
| 2026-03-25 | Reduce friction for good habits and increase it for bad ones. | Make the desired behaviour the path of least resistance: lay out your running shoes the night before, keep healthy snacks at eye level, or delete distracting apps from your home screen. The more effort a behaviour requires, the less likely it is to happen, so design your environment to work for you rather than against you. | self-help, habits |
| 2026-03-26 | Use Bash parameter expansion `${var//pattern/replacement}` for global substitution without calling `sed`. | A single slash `${var/pattern/replacement}` replaces the first match; a double slash replaces all. The pattern and replacement can use `$'...'` quoting for special characters, e.g. `${FOOBAR//$'\n'/_}` replaces every newline with an underscore. | shell, bash, reminder |
| 2026-03-27 | Use `${BASH_SOURCE[0]}` instead of `$0` to get the script's own path. | `$0` changes when a script is sourced (it becomes the caller's name), but `${BASH_SOURCE[0]}` always refers to the file where the code is written. | shell, bash, reminder |
| 2026-03-28 | Use a here-string `<<< "$var"` instead of `echo "$var" \| cmd`. | A here-string feeds a string directly to a command's stdin without spawning a subshell for `echo` and a pipe, e.g. `wc -w <<< "$sentence"`. | shell, bash, reminder |
| 2026-03-29 | If you always do more than is expected of you, you will always be expected to do more. | Exceeding expectations is a double-edged sword: it demonstrates your capability but quietly resets the baseline so that exceptional effort becomes the new normal. Be deliberate about when you go above and beyond. | reminder, self-help |
| 2026-03-30 | Compare yourself to who you were yesterday, not to who someone else is today. | Measuring progress against others invites envy and discouragement because you are comparing yourself with their highlight reel. The only meaningful benchmark is your own trajectory. | self-help, reminder |
| 2026-03-31 | Use `readRDS(url("https://example.com/file.rds", "rb"))` to load an RDS file directly from a URL. | Wrapping a URL in `url(..., "rb")` opens a binary connection that `readRDS()` can read from directly, so you can skip downloading the file to disk first. | rstats, reminder |
| 2026-04-01 | Exclude `.map` files from your NPM package by adding them to `.npmignore` or using the `files` field in `package.json`. | Source maps (`.js.map`) are debug files that map minified JavaScript back to original TypeScript. They are useful during development but should never ship to production or be published to NPM, as they expose your original source code and bloat the package. | javascript, typescript, reminder |
| 2026-04-02 | Use spaced repetition to retain what you learn. | Review material at increasing intervals, e.g. after 1 day, 3 days, 1 week, then 2 weeks, to exploit the spacing effect. Anki is a free, open-source flashcard app that automates this schedule for you, but even a simple calendar reminder works. | self-help, learning |
| 2026-04-03 | A 2 by 2 contingency table can help illustrate marginal and conditional probability. | Marginal probabilities are the row and column proportions, and conditional probabilities are the respective ratios on rows and columns. | learning |
